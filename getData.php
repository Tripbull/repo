...
switch($opt){
		...
		if($result->code == 200){
			...
			for($i=0;$i<count($objxml->product);$i++){
				...
				//if(!in_array((int)$objxml->product[$i]->id, array($connect->basic12,$connect->basic24,$connect->pro12,$connect->pro24,$connect->enterprise12,$connect->enterprise24))){
					$price = $i;//money_format('%i',(int)$objxml->product[$i]->price_in_cents/100);
	...
	case 'getrate':
		...
		if(mysql_num_rows($addnewfield) < 1)
			mysql_query("ALTER TABLE `businessCustom`  ADD `isselfie` TINYINT NOT NULL DEFAULT '0'  AFTER `fbpost`");
		$sql = "SELECT c.customPlaceId as placeId,p.profilePlaceId, p.businessName, p.nicename, p.category, p.address, p.city, p.country, p.zip, p.contactNo, p.showmap,c.messageBox,c.item2Rate,c.settingsItem,p.websiteURL,c.selectedItems,c.button,c.backgroundImg,c.reviewPost,c.logo,c.backgroundcolor,c.backgroundFont,c.ratingText,c.fbpost,c.email_alert,c.isselfie,g.state,g.productId,g.suspend,l.subscribe,c.optsocialpost,c.redirect,c.taglineselfie FROM businessProfile AS p
